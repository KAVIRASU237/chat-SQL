...
[**Get Started**](#-quick-start) · [**Features**](#-features) · [**Architecture**](#-architecture) · [**Screenshots**](#-screenshots) · [**Contributing**](#-contributing)
...
graph TD
    %% User Layer
    User([User / Admin]) -- Natural Language Query --> API

    %% Backend Container
    subgraph FastAPI ["FastAPI Backend Environment"]
        API[API Endpoints] 
        
        subgraph Security ["Security & Context Layer"]
            Auth{{"JWT Auth & RBAC Guard"}}
            RAG[("FAISS Vector Store<br/>(RAG Layer)")]
            Valid{"SQL Validator<br/>(Read-Only Enforcer)"}
        end

        subgraph Core ["Intelligence Engine"]
            LLM[["Ollama Local LLM<br/>(Mistral-7B / Llama3)"]]
        end

        subgraph Data ["Execution & Storage"]
            DB[("SQLite Database")]
        end

        subgraph Output ["Insight & Export"]
            Chart["Matplotlib<br/>Chart Generation"]
            AI["AI Narrative<br/>Insights"]
            CSV["CSV Export"]
        end
    end
    ...
    %% Styling
    style User fill:#6c5ce7,color:#fff,stroke:#333,stroke-width:2px
